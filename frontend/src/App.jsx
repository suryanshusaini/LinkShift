...
// ─── Mock helper (remove when backend is live) ───────────────────────────────
function mockShortenUrl(url) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const code = Math.random().toString(36).slice(2, 8);
      resolve({ shortUrl: `https://lnks.ft/${code}`, originalUrl: url });
    }, 1000);
  });
}
...
export default function App() {
  ...
  async function handleShorten(e) {
    ...
    setLoading(true);
    try {
      // 🔌 Swap mockShortenUrl → shortenUrl when backend is live
      const data = await mockShortenUrl(url.trim());
  ...
  return (
    <div
      ...
      style={{ backgroundColor: '#F0F5DF', fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}
    >
      {/* Card */}
      <div
      ...
      >
        ...
        {/* Form */}
        <form onSubmit={handleShorten} className="flex flex-col gap-4" noValidate>
          {/* URL Input */}
          <div className="relative">
            ...
            <input
              ...
              onFocus={e => { e.target.style.border = '1.5px solid #01949a'; e.target.style.backgroundColor = '#ffffff'; }}
              onBlur={e => { e.target.style.border = '1.5px solid #dbe8d8'; e.target.style.backgroundColor = '#dbe8d8'; }}
            />
          </div>
          ...
          <button
            ...
            onMouseEnter={e => { if (!loading) e.target.style.backgroundColor = '#017f84'; }}
            onMouseLeave={e => { e.target.style.backgroundColor = '#01949a'; }}
          >
            {loading ? (
          ...
          <div id="result-box" className="flex flex-col gap-3">
            ...
            >
              ...
              <button
                ...
                onMouseEnter={e => { if (!copied) { e.currentTarget.style.backgroundColor = '#c3d9bf'; } }}
                onMouseLeave={e => { if (!copied) { e.currentTarget.style.backgroundColor = '#dbe8d8'; } }}
              >
